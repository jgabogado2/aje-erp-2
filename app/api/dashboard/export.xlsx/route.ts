...
import { buildDashboardExcel } from '@/lib/reports/dashboard-excel';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const buffer = await buildDashboardExcel(data, query.year ?? year);
    return new Response(toResponseBody(buffer), {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="dashboard-summary-${query.year ?? year}-${timestampForFilename()}.xlsx"`,
